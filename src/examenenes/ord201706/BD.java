...
//Documentaci�n particular de foreign keys en sqlite en https://www.sqlite.org/foreignkeys.html

/** Clase de gesti�n de base de datos del sistema de ficheros e historial de cambios
...
public class BD {
	...
	public static void main(String[] args) {
		Object resp = null;
		do {
			resp = JOptionPane.showInputDialog( null, "Selecciona operaci�n de base de datos a realizar", "Gesti�n b�sica de BD de ficheros",
			...
			if ("Sacar BD a consola".equals(resp)) {
				...
				int resp2 = JOptionPane.showConfirmDialog( null, "�Confirmas que quieres borrar la base de datos?  Se perder� toda su informaci�n", "Confirmaci�n borrado", JOptionPane.YES_NO_OPTION );
		...
		try {
			...
			try {
				statement.executeUpdate("create table fichero " +
					...
					+ ", tamanyo bigint"          // Tama�o del fichero en bytes
					+ ", fecha bigint"            // Fecha de �ltima modificaci�n del fichero (msgs. desde 1/1/1970)
					+ ", ultRevision bigint"      // Fecha de �ltima revisi�n del fichero (msgs. desde 1/1/1970)
					+ ")" );
			...
			try {
				statement.executeUpdate("create table historiaCambios " +
					...
					+ ", tamanyo bigint"          // Tama�o anterior del fichero en bytes
					+ ", fecha bigint"            // Fecha anterior de �ltima modificaci�n del fichero (msgs. desde 1/1/1970)
					+ ", ultRevision bigint"      // Fecha de �ltima revisi�n del fichero (msgs. desde 1/1/1970)
					+ ")" );
			} catch (SQLException e) {} // Tabla ya existe. Nada que hacer
	...
	/** A�ade un fichero a la tabla abierta de BD, usando la sentencia INSERT de SQL
	 * @param st	Sentencia ya abierta de Base de Datos (con la estructura de tabla correspondiente al fichero)
	 * @param file	Fichero a a�adir en la base de datos
	 ...
	 * @param st	Sentencia ya abierta de Base de Datos (con la estructura de tabla correspondiente al fichero)
	 * @param codigoSelect	Sentencia correcta de WHERE (sin incluir la part�cula where) para filtrar la b�squeda (vac�a si no se usa)
	...
	/** A�ade un historial a la tabla abierta de BD, usando la sentencia INSERT de SQL
	 ...
	 * @param tamanyo	Tama�o anterior en bytes del fichero que ha cambiado
	 * @param fecha	Fecha anterior de fichero (msgs. desde 1/1/1970)
	 * @param ultRevision	Fecha de �ltima revisi�n de ese cambio (msgs. desde 1/1/1970)
	 * @return	true si la inserci�n es correcta, false en caso contrario
	 */
	...
	public static ArrayList<Fich> historiaSelect( Statement st, String path ) {
		String sentSQL = "";
		ArrayList<Fich> ret = new ArrayList<>();
		if (path==null || path.isEmpty()) return ret;  // Si no hay path se devuelve vac�o
		try {
	 ...
	 * @return	String asegurado, sin caracteres extra�os, sin saltos de l�nea ni tabuladores, con las comillas convertidas en dobles comillas
	 */
	...
	public static Logger getLogger() {
		initLogger();  // Crea el logger si no est� iniciado
		return logger;
	}
		// M�todo local - crea el logger si no est� iniciado
		private static void initLogger() {
			if (logger==null) {  // Logger por defecto local:
				logger = Logger.getLogger( BD.class.getName() );  // Nombre del logger - el de la clase
				logger.setLevel( Level.ALL );  // Loguea todos los niveles
				logger.setUseParentHandlers( ACTIVAR_LOG_A_CONSOLA_DE_ERROR ); // Conecta o desconecta la salida est�ndar a System.err
				try {
