...
public class PanelDibujoBancoDePruebas extends JPanel {
	...
	// *******************************************************************
	// Datos y m�todos generales para hacer un panel con dibujado gr�fico permanente
	// *******************************************************************
	
	private BufferedImage imagen;  // Imagen a dibujar en el panel

	/** Crea un panel de dibujado
	 * @param tamHor	Tama�o m�ximo horizontal de dibujo (en p�xels)
	 * @param tamVer	Tama�o m�ximo vertical de dibujo (en p�xels)
	...
	public void pintaFondo( Color cF ) {
		...
		// g2.fillRect( 0, 0, imagen.getWidth(), imagen.getHeight() );  // Hacerlo as� si se quiere ver s�lo parte de la imagen pero inicializar toda
	}
...
//}
	...
	// *******************************************************************
	// Datos y m�todos de dibujado especializado para el banco de pruebas
	// *******************************************************************
	...
	ArrayList<Integer> tamanyos;    // Arraylist de los tama�os
	ArrayList<Color> colores;  // Arraylist de colores
	long maxEspacio;  // Valor m�ximo a dibujar de espacio (en bytes)
	long maxTiempo;   // Valor m�ximo a dibujar de tiempo (en msgs)
	int maxTamanyo;   // Valor m�ximo a dibujar de tama�o
	
	Graphics2D g2;  // Objeto gr�fico en el que dibujar
	int origenX;  // Origen X
	int origenYEspacios;  // Origen Y (espacio)
	int origenYTiempos;  // Origen Y (tiempo)
	int ESPACIO_IZQDA_X = 40; // P�xels libres por la izquierda
	int ESPACIO_DCHA_X = 40; // P�xels libres por la derecha
	int ESPACIO_ABAJO_Y = 20; // P�xels libres por abajo
	int RADIO_CIRCULO_PUNTO = 3;  // P�xels de radio de circulito
	int alto;   // Altura de dibujo de cada gr�fico (en pixels)
	int ancho;  // Anchura de dibujo de cada gr�fico (en p�xels)
	...
	public void iniciarDibujo( int tamMax, String[] nombresPruebas ) {
		...
		iniciarColores();
		maxEspacio = 10;  // Empezamos en 10 bytes (luego se ir� redimensionando)
		maxTiempo = 10;  // Empezamos en 10 msgs (luego se ir� redimensionando)
		maxTamanyo = tamMax;
	...
	/** Reinicia el dibujo con distinto tama�o de lienzo (llamar cuando cambie el tama�o del panel),
	 * pero el resto de datos siguen como estuvieran.
	 */
	public void reiniciarDibujo() {
		if (tiempos!=null) {  // Si ya est� inicializado el dibujo
	...
	private void iniciarColores() {
		...
		while (colores.size() < tiempos.length ) {  // Si hay m�s de 3 colores se a�aden aleatoriamente
			colores.add( new Color( r.nextInt(240), r.nextInt(240), r.nextInt(240) ) );  // Se llega a 240 para no usar por azar el blanco (255,255,255) que se confundir�a con el fondo
		}
	}
