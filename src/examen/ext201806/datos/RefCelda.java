...
public class RefCelda implements Serializable {
	 ...
	 * @param f	Número de fila de la celda (0 a n-1)
	 * @param c	Número de columna de la celda (0 a n-1)
	 */
	...
	/** Construye la referencia a una celda dada en formato texto. Si la referencia es inválida
	 * lanza una excepción
	 * @param refTexto	Referencia en formato letra + número (por ejemplo "A1", "C3", "AF95"...)
	 * 	La letra hace referencia a la columna (A=0, B=1, etc.) y el número a la columna (1=0, 2=1...) 
	 * @throws NumberFormatException	Si la referencia es inválida
	 */
		...
		try {
			...
			do {
				...
				if (letra>='a' && letra<='z') {
					...
					throw new Exception();  // Error en construcción
				}
				refCol = refCol.substring(1);
			} while (refCol.length()>0);
			fila = Integer.parseInt(refFil) - 1;
		} catch (Exception e) {
			throw new NumberFormatException( "Referencia de celda inválida: " + refTexto );
		}
	}
	...
	/** Devuelve el nombre en letra de la columna indicada en número, "" si hay cualquier error
