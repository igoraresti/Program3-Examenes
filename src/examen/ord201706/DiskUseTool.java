...
import static examen.ord201706.VentanaDiskUseTool.*;
...
public class DiskUseTool {
	...
	public void visualizarEspacioEnDiscoConJTree( String... dirIni ) {
		for (String dir : dirIni) {
			...
			vent.getModeloArbol().nodeChanged( vent.getRaiz() );  // Lanza evento de modificaciÃ³n en el modelo
		}
	}

	// TAREA Hilo - atributo y método público
	
	private ConteoDir recorridoUnidad( File ficODir, int nivel, NodoConTamanyo padre ) {
		// TAREA Hilo - parada de recorrido
		if (ficODir==null) return new ConteoDir();  // No hay ficheros, no hay carpetas, tamaño cero bytes
		if (!ficODir.isDirectory()) { // Si es fichero se devuelve tal cual
			procesaFichero( ficODir );
			return new ConteoDir( null, 1, 0, ficODir.length() );  // 1 fichero, no hay carpetas, tamaño bytes del fichero
		}
		// Si es directorio, proceso recursivo
		File[] files = ficODir.listFiles();
		String nom = ficODir.getName();
		if (nom.isEmpty()) nom = ficODir.getAbsolutePath();  // Caso especial para raíz del disco
		final NodoConTamanyo nodoNuevo = vent.anyadeNodoHijo( nom, new ConteoDir(ficODir, 0, 1, 0L), padre );  // No hay ficheros, una carpeta -aún sin contenido-, tamaño cero bytes
		if (files==null || files.length==0) return nodoNuevo.datos;  // No hay ficheros, una carpeta -sin contenido-, tamaño cero bytes
		...
		for (File file : al) {
			...
			SwingUtilities.invokeLater( new Runnable() {
				@Override
				public void run() {
					vent.getModeloArbol().nodeChanged( nodoNuevo );  // Lanza evento de modificación en el modelo
				}
			});
		}
		nodoNuevo.datos.setNumCarpetas( nodoNuevo.datos.getNumCarpetas() + 1 );  // Actualizar el número de carpetas para retornar (las que haya dentro + la actual)
		SwingUtilities.invokeLater( new Runnable() { // Colapsar la carpeta y así el árbol va teniendo un tamaño aceptable en la ventana
			@Override
			public void run() {
	...
	// ---------------------------------
	// Métodos de secuencia de recorrido de ficheros
	// ---------------------------------
	...
	// Se ejecuta después de acabar el recorrido
