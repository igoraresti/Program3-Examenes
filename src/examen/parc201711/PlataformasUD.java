...
public class PlataformasUD {
	...
	private void procesarTeclado() {
		...
		} else {  // Tecla cursor izquierda no está pulsada
			gestionTeclas( "LEFT", false );
		}
		if (ventana.isTeclaPulsada( KeyEvent.VK_RIGHT )) {
			gestionTeclas( "RIGHT", true );
		} else {  // Tecla cursor derecha no está pulsada
			gestionTeclas( "RIGHT", false );
		}
		if (ventana.isTeclaPulsada( KeyEvent.VK_UP )) {
			gestionTeclas( "UP", true );
		} else {  // Tecla cursor arriba no está pulsada
			gestionTeclas( "UP", false );
		}
	}
	
		// TODO TAREA 3 - Atributos de estructuras de datos para control de estadísticas
		// TODO TAREA 3 - Detección de teclas para estadísticas
		// Se llama a este método cada iteración de bucle de juego con las tres teclas de control
		// ("UP", "LEFT", "RIGHT") en cada caso con pulsada=true o false según su estado de pulsación
...
//		} while (hayChoques && numIteracion<=3);
	...
	private void procesaChoque( ObjetoMovil objeto1, ObjetoMovil objeto2, Point2D choque, int numIteracion ) {
		...
		if (CALC_CHOQUE_EXACTO && objeto1 instanceof UDcito && objeto2 instanceof Pelota) {
			...
			double milisChoque = calcularChoqueExacto( 0, ventana, ud, pelota, MILIS_POR_MOVIMIENTO );  // Cálculo recursivo de choque
			ventana.borra();
			ud.dibuja( ventana );
			pelota.dibuja( ventana );
			ventana.setDibujadoInmediato( false );  // Volver al modo de dibujado estándar
		} */ // TAREA 4
	...
	private void test( ArrayList<ObjetoMovil> listaObjetos ) {
		// TODO TAREA 3 - Visualización de estructuras de datos para control de estadísticas
	}
	...
	private double calcularChoqueExacto( int numLlamadas, VentanaGrafica ventana, UDcito ud, Pelota pelota, double milisMovimiento ) {
		// Mover la mitad
		// ...
		// ud.dibuja( ventana );      // Si quieres en cada prueba recursiva para ver dónde está ahora udcito
		// pelota.dibuja( ventana );  // y dónde está ahora la pelota
		// ventana.esperaAClick();    // Si quieres que en cada paso recursivo se pause el juego hasta que se haga click
		// Proceso recursivo - una mitad u otra según si ahora hay o no choque
		// ...
